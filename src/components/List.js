...
export function ListElement() {
    ...
    const fetchData1 = async () => {
      try {
        const response1 = await fetch(
          `https://backend-production-b756.up.railway.app/viewterrainpersonne/findTerrainPersonneValider/${userId}`
        );
  ...
  const validerTerrain = async (idTerrain) => {
    try {
      const response1 = await fetch(
        `https://backend-production-b756.up.railway.app/terrain/validerTerrain/${idTerrain}`,
        {
          method: "POST",
        }
      );
  ...
  const handleAddParcelle = (idTerrain) => {
    navigate(`/Details/${idTerrain}`);
  };
            ...
            {data ? (
              data.map((item) => (
                <tr className="content" key={item.idTerrain}>
                  ...
                  <td>
                    <a onClick={() => handleLinkClick(item.idTerrain)}>
                      <IoIosCreate className="iconM" />
                    </a>
                  </td>
                </tr>
              ))
              ...
              {data1 ? (
                data1.map((item1) => (
                  <tr className="content" key={item1.idTerrain}>
                    ...
                    <td>
                      <a
                        onClick={() => handleAddParcelle(item1.idTerrain)}
                      >
                        <IoIosCreate className="iconM" />
                      </a>
                    </td>
